...
all: $(PDFS)
	...
	@printf "Writing index.html file ..."
	...
	@for PDF in $? ; do echo "<li><a href=\"$$PDF\">$$(cat $${PDF/.pdf/.tit})</a></li>" >> public/index.html ; done
...
clean:
	rm -rf $(SOURCES:tex=aux) $(SOURCES:tex=log) $(SOURCES:tex=out) $(SOURCES:tex=pdf) $(SOURCES:tex=tit) public
